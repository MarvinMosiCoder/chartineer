...
**Correction to the same section:** it describes imported trades as offering a "real-versus-simulated comparison". No such comparison exists. `imported_trades` and `market_backtest_positions` are deliberately isolated — no shared foreign keys, no shared report code path — and [Imported trades](../../developer/imported-trades.md) names that separation as intentional, stating any future comparison belongs in a new service reading both sources separately. Elite grants the import and its own reporting, nothing that joins the two datasets.
