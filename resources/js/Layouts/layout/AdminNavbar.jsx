...
export default function AdminNavbar() {
    // `auth` is a prop; `url` is on the page object itself. Destructuring both
    // from usePage() silently yields an undefined auth and an empty nav.
    const page = usePage();
    ...
    return <header className={`flex h-14 items-center border-b px-3 sm:px-4 ${isDark ? 'border-[#2a2e39] bg-[#131722] text-[#d1d4dc]' : 'border-slate-200 bg-white text-slate-800'}`}>
        {/* Hamburger sits left of the logo with a left-anchored dropdown, the same
            placement TraderNavbar uses, so both shells behave identically below lg. */}
        <div data-nav-menu className="relative lg:hidden">
            <button type="button" onClick={()=>setShowModuleMenu((visible)=>!visible)} className="mr-2 rounded-md p-2 hover:bg-white/10" aria-label="Navigation" aria-expanded={showModuleMenu}><Menu size={18}/></button>
            {showModuleMenu && <div className={`absolute left-0 top-12 z-[230] max-h-[calc(100dvh-5rem)] w-64 overflow-y-auto p-1.5 ${panelClass}`}>
                {userMenus.map((item) => <Link key={`m-menu-${item.id ?? item.name}`} href={menuHref(item)} onClick={closeMenus} className={menuItemClass(isActive(item))}><MenuIcon icon={item.icon}/>{item.name}</Link>)}

                {/* Group parents become labelled sections rather than nested taps —
                    there is vertical room here, so a second tap would be friction. */}
                ...
                {overflowEntries.map((item) => <Link key={`m-single-${item.id ?? item.name}`} href={menuHref(item)} onClick={closeMenus} className={menuItemClass(isActive(item))}><MenuIcon icon={item.icon}/>{item.name}</Link>)}</>}
