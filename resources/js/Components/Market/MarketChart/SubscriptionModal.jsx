...
export default function SubscriptionModal({ onClose, onTrialActivated }) {
  ...
  // Teal-300 reads fine on the dark shell but drops to ~1.3:1 on the light one, where
  // `shell` is white and `surface` slate-50 — hence a darker teal for that side.
  const accent = dark ? 'text-[#5eead4]' : 'text-teal-700';
  ...
  return <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 p-2 backdrop-blur-sm sm:p-3">
    <section className={`flex max-h-[calc(100dvh-1rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border shadow-2xl sm:max-h-[calc(100dvh-1.5rem)] ${shell}`}>
      <header className={`flex shrink-0 items-start justify-between border-b px-4 py-3 sm:px-5 ${surface}`}>
        <div><div className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.18em] ${accent}`}><Sparkles size={13}/>Replay access</div><h2 className="mt-0.5 text-xl font-bold">Build your trading practice</h2><p className="mt-0.5 text-xs text-[#787b86]">Activate your free week or purchase one-time access through our secure payment provider.</p></div>
      ...
      <div className="min-h-0 overflow-y-auto p-4 sm:p-5">
        {loading ? <div className="py-12 text-center text-[#787b86]">Loading plans…</div> : <div className="grid gap-2.5 md:grid-cols-3">{plans.map(plan => {
          ...
          return <button key={plan.id} type="button" disabled={!configured || locked} onClick={() => setSelectedCode(plan.code)} className={`relative flex flex-col items-stretch justify-start rounded-xl border p-3 text-left disabled:opacity-60 ${chosen ? 'border-[#2dd4bf] bg-[#2dd4bf]/10 shadow-[0_0_0_1px_#2dd4bf]' : surface}`}>
            ...
            <div className="flex items-start gap-2"><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${dark ? 'bg-[#2dd4bf]/10' : 'bg-teal-100'} ${accent}`}><Icon size={15}/></span><div className="min-w-0"><span className={`block text-[9px] font-bold uppercase tracking-[.18em] ${accent}`}>{plan.tier_name}</span><h3 className="font-bold leading-tight">{plan.name}</h3><p className="text-xl font-bold leading-tight">{configured ? `${plan.currency} ${Number(plan.price).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : 'Price pending'}</p></div></div><p className="mt-1 text-[11px] text-[#787b86]">{plan.duration_days} days · one-time payment</p><p className="mt-1 line-clamp-2 text-[11px] text-[#787b86]">{plan.description}</p>
            ...
            {(plan.features ?? []).length > 0 && <div className={`mt-1.5 grid gap-1 border-t pt-1.5 ${dark ? 'border-white/5' : 'border-slate-200'}`}>{plan.features.map(feature => <div key={feature} className="flex items-start gap-1.5 text-[11px] text-[#787b86]"><Check size={11} className="mt-0.5 shrink-0 text-[#787b86]"/><span>{feature}</span></div>)}</div>}
            {/* mt-auto, so the three cards' CTAs line up on one row no matter how
                many capabilities each lists above them. */}
            <div className={`mt-auto flex items-center gap-1.5 pt-2 text-[11px] font-semibold ${accent}`}>{locked ? <><Lock size={12}/>{isCurrent ? 'Active plan' : 'Already included'}</> : <><Check size={12}/>{chosen ? 'Selected' : (isUpgrade ? 'Upgrade' : 'Select plan')}</>}</div>
          </button>;
        })}</div>}
        ...
        {!readOnly && isUpgrade && <div className={`mt-2.5 rounded-lg p-2 text-xs ${dark ? 'bg-[#2dd4bf]/10' : 'bg-teal-50'} ${accent}`}>Your {activeAccess?.tierName ?? 'current'} access runs until {new Date(activeAccess.endsAt).toLocaleString()}. Upgrading keeps those remaining days and adds the new plan's full duration on top.</div>}
        {!readOnly && paidTier === 0 && activeAccess?.kind === 'trial' && <div className={`mt-2.5 rounded-lg p-2 text-xs ${dark ? 'bg-[#2dd4bf]/10' : 'bg-teal-50'} ${accent}`}>Your free trial runs until {new Date(activeAccess.endsAt).toLocaleString()}. Buying now does not cut it short — your paid plan starts the moment the trial ends.</div>}
